...
const ProjectDetailsComponent = ({project}: ProjectDetailsProps) => {
    ...
    return (
        <article className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-700">
            ...
            <div className="border-t border-gray-100 pt-12 flex justify-end">
                <Link href={`/projects/${nextProject.title?.toLowerCase()}`} className="group text-right">
                    ...
                    <h3 className="text-2xl font-bold flex items-center gap-2 group-hover:underline decoration-2 underline-offset-4">
                        {nextProject.title} <ArrowLeft className="w-5 h-5 rotate-180 transition-transform group-hover:translate-x-1" />
                    </h3>
                </Link>
            </div>
        </article>
